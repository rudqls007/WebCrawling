    ...
    * 크롤링 순서
        ...
        반복문 사용
            <a> 태그의 href 속성을 읽어 옴
                ...
                split()
            <img> 태그
                title 속성을 읽어 와서 제목으로 처리한다
        리스트에 추가함
        해당 이미지를 각 요일 폴더에 이미지로 저장함

        데이터 프레임 만들어 CSV 파일로 저장한다.
'''
...
myfolder = 'c:\\Users\\rudql\\Desktop\\ezen_webtoon\\'
...
myframe = DataFrame(mylist, columns=['타이틀 번호', '요일', '제목', '랑쿠'])
